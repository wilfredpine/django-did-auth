...
from django_sauth.security.audit.logger import log_register_success, log_register_failure
...
def register_view(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            log_register_success(request, user)                    # ← Good
            messages.success(request, "Registration successful! Please check your email to activate your account.")
            return redirect("sauth:verification_sent")
        else:
            log_register_failure(request, form.errors)             # ← Good
            # Optionally add user-facing error
            messages.error(request, "Please correct the errors below.")
    else:
        form = FormClass()

    return render(request, "sauth/register.html", {"form": form})